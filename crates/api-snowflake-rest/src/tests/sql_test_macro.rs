...
pub fn error_from_response(response: &JsonResponse) -> String {
...
impl SqlTest {
    ...
    fn create_access_token(&self, addr: std::net::SocketAddr) -> String {
        ...
        let jwt_secret = &self.server_cfg.as_ref()
            .map_or_else(|| TEST_JWT_SECRET.to_string(), |cfg| cfg.auth.jwt_secret.clone());
        ...
        let jwt_token = create_jwt(&jwt_claims, jwt_secret)
...
pub async fn sql_test_wrapper<F>(sql_test: SqlTest, check_response_cb: F)
where
    F: Fn((String, String), &JsonResponse) -> bool,
{
    ...
    let skip_login_token = sql_test.skip_login.then(|| sql_test.create_access_token(server_addr));
    ...
    for (idx, sql) in sql_test.sqls.iter().enumerate() {
        ...
        let _ = check_response_cb((sql, sql_info), &snapshot);
...
macro_rules! sql_test {
    ($name:ident, $sql_test:expr) => {
        ...
        async fn $name() {
            ...
            let snapshot_cb = move |sql_info: (String, String), response: &JsonResponse| -> bool {
