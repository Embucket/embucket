...
use arrow_flight::{flight_service_server::FlightService, flight_service_server::FlightServiceServer};
...
impl FlightService for FlightServiceImpl {
    ...
    async fn do_get(&self, request: Request<Ticket>) -> Result<Response<Self::DoGetStream>, Status> {
        ...
        let plan = Plan::decode(ticket.ticket).map_err(|e| Status::invalid_argument(e.to_string()))?;
        ...
        catalog_push.register_schema("temp", self.temp_schema.clone()).map_err(to_tonic_err)?;
        ...
        catalog_bench.register_schema("benchmark_schema", self.temp_schema.clone()).map_err(to_tonic_err)?;
        ctx.register_catalog("benchmark_database", catalog_bench);
        let logical_plan = from_substrait_plan(&ctx.state(), &plan).await.map_err(to_tonic_err)?;
    ...
    async fn do_put(&self, _request: Request<Streaming<FlightData>>) -> Result<Response<Self::DoPutStream>, Status> {
        ...
        let first = stream.message().await?.ok_or_else(|| Status::invalid_argument("empty DoPut stream"))?;
        let schema = std::sync::Arc::new(Schema::try_from(&first).map_err(|e| Status::invalid_argument(e.to_string()))?);
        ...
        let table_name = if let Some(desc) = &first.flight_descriptor {
            if !desc.path.is_empty() {
                desc.path.last().cloned().unwrap_or_else(|| "temp".to_string())
            } else {
                "temp".to_string()
            }
        } else {
        ...
        let table = datafusion::datasource::MemTable::try_new(schema.clone(), vec![batches]).map_err(to_tonic_err)?;
        self.temp_schema.register_table(table_name, std::sync::Arc::new(table) as std::sync::Arc<dyn TableProvider>).map_err(to_tonic_err)?;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let service = FlightServiceImpl { temp_schema: std::sync::Arc::new(MemorySchemaProvider::new()) };
