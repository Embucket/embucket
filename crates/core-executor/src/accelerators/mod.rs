...
pub mod velox_impl {
    ...
    fn plan_has_local_files(plan: &sp::Plan) -> bool {
        ...
        fn rel_has_local_files(rel: &sp::Rel) -> bool {
            match rel.rel_type.as_ref() {
                ...
                Some(sp::rel::RelType::Join(j)) => {
                    rel_opt_box_has_local_files(&j.left) || rel_opt_box_has_local_files(&j.right)
                }
                ...
                Some(sp::rel::RelType::Cross(c)) => {
                    rel_opt_box_has_local_files(&c.left) || rel_opt_box_has_local_files(&c.right)
                }
                _ => false,
            }
        }
    ...
    impl VeloxAccelerator {
        pub fn new(cfg: VeloxConfig) -> Self {
            Self { _cfg: cfg }
        }
    }
    ...
    impl ExternalAccelerator for VeloxAccelerator {
        ...
        async fn execute(
            ...
            let substrait =
                datafusion_substrait::logical_plan::producer::to_substrait_plan(plan, state)
            ...
            let stream = session
                .execute_substrait_to_arrow_stream(&bytes)
                .map_err(|e| DataFusionError::Execution(e.to_string()))
                .context(crate::error::DataFusionSnafu)?;
            ...
            let s: SendableRecordBatchStream =
