...
async fn main() {
    ...
    let warehouse_id = "94098990-fc8c-48f6-a50f-3e8e309b29ad";
    let database_name = "datasets";
    let warehouse_name = "embucket";
    ...
    let query1 = format!("CREATE OR REPLACE TABLE {initial_table_ident} (i INTEGER, p CHAR(1), o INTEGER);");
    let records: Vec<RecordBatch> = SqlExecutor::new(ctx.clone())
        .query(&query1.to_string(), &warehouse_name.clone().to_string())
        .await.unwrap()
        .into_iter()
    ...
    let query4 = format!("CREATE OR REPLACE table {result_table_ident} AS
        ...
        WHERE p = 'A' OR p = 'B' AND r.qualify_alias = 1)
SELECT p.i, p.p, p.o FROM prep_table)");
