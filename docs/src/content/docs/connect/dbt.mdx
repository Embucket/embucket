...
[dbt](https://docs.getdbt.com/) provides a SQL-first transformation tool for analytics engineering. The `dbt-embucket` adapter connects to Embucket by invoking the Lambda function directly through AWS APIs, so no public endpoint proves necessary.
...
6. **Verify the result**
   ...
   <Aside type="tip">The adapter talks to Lambda by ARN. You don't need a public endpoint.</Aside>
