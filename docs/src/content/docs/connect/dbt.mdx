...
[dbt](https://docs.getdbt.com/) is a SQL-first transformation tool for analytics engineering. The `dbt-embucket` adapter connects to Embucket by invoking the Lambda function directly through AWS APIs, so no public endpoint is necessary.
...
6. **Verify the result**
   ...
   <Aside type="tip">The adapter talks to Lambda by ARN. You do not need a public endpoint.</Aside>
