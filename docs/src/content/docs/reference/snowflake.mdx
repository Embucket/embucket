...
Embucket uses [Apache DataFusion](https://datafusion.apache.org/) as its query engine -- an Apache Arrow-native analytical query engine. The project targets full SQL dialect compatibility with Snowflake. Embucket supports a growing set of Snowflake SQL features. DataFusion also includes some built-in functions that Snowflake doesn't provide.
...
Snowflake operates as a managed analytics database built on FoundationDB and object storage. See the [Snowflake whitepaper](https://www.cs.cmu.edu/~15721-f24/papers/Snowflake.pdf) for details on its architecture. Embucket offers an open-source alternative that uses Apache DataFusion, Apache Iceberg, Apache Arrow, and Parquet.
...
<Aside type="caution">
  The "Current limitations" section below lists features that differ from Snowflake or lack support.
</Aside>
...
<Aside type="note">
  Numeric precision handling remains under active development. Future releases aim to match
  Snowflake's dynamic precision Decimal types.
</Aside>
