...
Embucket uses [Apache DataFusion](https://datafusion.apache.org/) as its query engine -- an Apache Arrow-native analytical query engine. The goal is full SQL dialect compatibility with Snowflake. Embucket supports a growing set of Snowflake SQL features. DataFusion also includes some built-in functions that Snowflake does not provide.
...
<Aside type="caution">
  The "Current limitations" section below lists features that differ from Snowflake or are not yet
  supported.
</Aside>
...
<Aside type="note">Numeric precision handling is under active development. Future releases aim to match Snowflake's dynamic precision Decimal types.</Aside>
