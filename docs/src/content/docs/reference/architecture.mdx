...
Embucket runs as an AWS Lambda function (`embucket-lambda`) for production deployments. A local binary (`embucketd`) is available through Docker for evaluation. Both entry points share the same Snowflake-compatible API router.
...
<Aside type="tip">
See [Configuration](/deploy/configuration/) for the full list of metastore options and environment variables.
</Aside>
...
Embucket executes Snowflake-compatible SQL through Apache DataFusion. Query execution is single-node per request. Each invocation handles a complete query independently. There is no distributed query plan across nodes.
