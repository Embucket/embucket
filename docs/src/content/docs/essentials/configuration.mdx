...
When loaded, Embucket will create a database `demo` with schema `tpch_10` and two tables `customer` and `lineitem`. It uses the `tpch` volume information in the configuration file to access data.
