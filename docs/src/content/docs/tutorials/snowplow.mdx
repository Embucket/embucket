...
Snowplow is an open source behavioral data platform that captures granular, event-level web analytics. In this tutorial you connect Snowplow's dbt packages to Embucket and produce three derived tables:
...
<Aside type="tip">
If you haven't deployed Embucket to Lambda before, complete the [AWS Lambda deployment](/deploy/aws-lambda/) guide first.
</Aside>
...
<Aside type="caution">
Deleting the stack removes the Lambda function and IAM role but doesn't remove data from your S3 Table Bucket. Delete the tables manually if you no longer need them.
</Aside>
