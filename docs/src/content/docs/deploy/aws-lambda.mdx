...
3. ### Deploy the Lambda function
   ...
   Download the pre-built Lambda zip and deploy it with the AWS CLI:
   ...
   <Aside type="tip">
     Replace the role ARN with your own execution role. The pre-built zip includes the
     `state-store-query` feature for DynamoDB-backed query state persistence.
   </Aside>
   ...
   If you prefer to build from source, clone the repository and use the Makefile:

   ```bash
   make -C crates/embucket-lambda deploy
   ```

   This requires the Rust toolchain and `cargo-lambda`. The Makefile accepts the following variables:
