...
This guide walks you through configuring AWS S3 table buckets as an external catalog in Embucket. At the moment, S3 table buckets is the only external catalog available. AWS S3 table bucket provides built-in table maintenance, automatic compaction, and optimization features.
...
- Configure an Embucket with S3 table bucket as an external catalog
- Exploring schema, tables and existing data
...
<Aside type="caution">
  Each S3 table bucket in Embucket maps to exactly one database. You specify 
...
In Embucket, volumes are used to store data and metadata. Currently all the configuration is static and defined in the configuration file. You specify the config file location with `--metastore-config` flag (or `METASTORE_CONFIG` environment variable).
...
- `type`: Volume type `s3-tables`. At the moment, this is the only type supported.
