...
description: Configure Embucket to read catalog metadata from an AWS S3 table bucket.
---
...
Use this guide when you want Embucket to read catalog metadata from AWS S3 Tables. In the current docs set, S3 table buckets are the documented external catalog path.
...
S3 table buckets map naturally to the current Embucket external catalog flow: one bucket ARN maps to one configured database.
...
- **Analytics example**: If you want a fuller dbt-based workflow on Lambda, read [Snowplow web analytics](/guides/snowplow/)
